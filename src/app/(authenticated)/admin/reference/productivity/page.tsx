  ...
  async function save() {
    ...
    const res = await fetch(`/api/admin/reference/productivity/${editingId}`, {
      ...
      body: JSON.stringify({
        ...
        mhPerUnitLow:
          draft.mhPerUnitLow !== '' && draft.mhPerUnitLow != null
            ? Number(draft.mhPerUnitLow)
            : null,
        mhPerUnitAvg: Number(draft.mhPerUnitAvg),
        mhPerUnitHigh:
          draft.mhPerUnitHigh !== '' && draft.mhPerUnitHigh != null
            ? Number(draft.mhPerUnitHigh)
            : null,
      ...
      toolbar={
        <div className="flex items-center gap-2">
          <div className="relative">
            ...
            <Input
              placeholder="Search scope / matchCode / keywords"
              value={search}
                ...
                editingId === e.id ? (
                  <tr key={e.id} className="border-t border-border bg-blue-500/5">
                    <td className="px-3 py-1.5">
                      <select
                        ...
                        onChange={(ev) => setDraft({ ...draft, divisionId: ev.target.value })}
                    ...
                    <td className="px-3 py-1.5">
                      <input
                        ...
                        onChange={(ev) => setDraft({ ...draft, scopeName: ev.target.value })}
                        className="h-7 w-full rounded border border-border bg-canvas px-2 text-[12px]"
                      />
                    </td>
                    <td className="px-3 py-1.5">
                      <input
                        value={draft.matchCode ?? ''}
                        onChange={(ev) => setDraft({ ...draft, matchCode: ev.target.value })}
                        placeholder="ELFCS"
                        className="h-7 w-24 rounded border border-border bg-canvas px-2 font-mono text-[11px] uppercase"
                      />
                    </td>
                    <td className="px-3 py-1.5">
                      <select
                        value={draft.uom ?? ''}
                        onChange={(ev) => setDraft({ ...draft, uom: ev.target.value })}
                        className="h-7 w-full rounded border border-border bg-canvas px-1 text-[12px]"
                      >
                        {UOMS.map((u) => (
                    ...
                    <td className="px-3 py-1.5 text-right">
                      <input
                        type="number"
                        step="0.0001"
                        value={draft.mhPerUnitLow ?? ''}
                        onChange={(ev) => setDraft({ ...draft, mhPerUnitLow: ev.target.value })}
                        className="h-7 w-20 rounded border border-border bg-canvas px-2 text-right font-mono text-[12px]"
                      />
                    </td>
                    <td className="px-3 py-1.5 text-right">
                      <input
                        type="number"
                        step="0.0001"
                        value={draft.mhPerUnitAvg ?? ''}
                        onChange={(ev) => setDraft({ ...draft, mhPerUnitAvg: ev.target.value })}
                        className="h-7 w-20 rounded border border-border bg-canvas px-2 text-right font-mono text-[12px]"
                      />
                    </td>
                    <td className="px-3 py-1.5 text-right">
                      <input
                        type="number"
                        step="0.0001"
                        value={draft.mhPerUnitHigh ?? ''}
                        onChange={(ev) => setDraft({ ...draft, mhPerUnitHigh: ev.target.value })}
                        className="h-7 w-20 rounded border border-border bg-canvas px-2 text-right font-mono text-[12px]"
                      />
                    </td>
                    <td className="px-3 py-1.5">
                      <select
                        value={draft.assumedTradeId ?? ''}
                        onChange={(ev) =>
                          setDraft({ ...draft, assumedTradeId: ev.target.value || null })
                        }
                        className="h-7 w-full rounded border border-border bg-canvas px-1 text-[12px]"
                      >
                  ...
                  <tr key={e.id} className="border-t border-border hover:bg-sunken/40">
                    ...
                    <td className="px-3 py-1.5 text-fg-default">
                      <button
                        onClick={() => {
                          ...
                          setDraft({
                            ...e,
                            mhPerUnitLow: e.mhPerUnitLow ?? '',
                            mhPerUnitAvg: e.mhPerUnitAvg,
                            mhPerUnitHigh: e.mhPerUnitHigh ?? '',
                          });
                        }}
                    ...
                    <td className="px-3 py-1.5 text-right font-mono text-fg-subtle">
                      {e.mhPerUnitLow !== null ? Number(e.mhPerUnitLow).toFixed(4) : '—'}
                    </td>
                    <td className="px-3 py-1.5 text-right font-mono">
                      {Number(e.mhPerUnitAvg).toFixed(4)}
                    </td>
                    <td className="px-3 py-1.5 text-right font-mono text-fg-subtle">
                      {e.mhPerUnitHigh !== null ? Number(e.mhPerUnitHigh).toFixed(4) : '—'}
                    </td>
                    ...
                    <td className="pr-2">
                      <div className="flex justify-end">
                        <Button
                          ...
                          variant="ghost"
                          className="h-7 w-7 p-0"
                          onClick={() => remove(e.id, e.scopeName)}
                        >
      ...
      <div className="w-full max-w-md rounded-lg border border-border bg-canvas p-5">
        ...
        <div className="mt-4 space-y-3 text-[12.5px]">
          <label className="block">
            <span className="block text-[10.5px] font-semibold uppercase text-fg-subtle">
              Scope name *
            </span>
            <Input
              value={form.scopeName}
              onChange={(e) => setForm({ ...form, scopeName: e.target.value })}
              placeholder="Fiber cement fascia install"
              autoFocus
            />
          </label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <label className="block">
              ...
              <Input
                ...
                step="0.0001"
                value={form.mhAvg}
                onChange={(e) => setForm({ ...form, mhAvg: e.target.value })}
                placeholder="0.055"
              />
            </label>
            <label className="block">
              ...
              <Input
                ...
                onChange={(e) => setForm({ ...form, matchCode: e.target.value.toUpperCase() })}
                placeholder="ELFCS"
        ...
        <div className="mt-5 flex justify-end gap-2">
          ...
          <Button
            disabled={!form.scopeName || !form.divisionId || !form.mhAvg}
            onClick={() =>
              onSubmit({
                scopeName: form.scopeName.trim(),
                divisionId: form.divisionId,
                uom: form.uom,
                mhPerUnitAvg: parseFloat(form.mhAvg),
                matchCode: form.matchCode.trim() || null,
                assumedTradeId: form.tradeId || null,
              })
            }
          >
            Create
